...
void* State_Alloc(u64 _Size, u64 _Align)
{
	...
	for (char* c = (char*)State->Data; (u64)c < (u64)State->DataEnd; c++)
	{
		...
		if (RequiredSizeCounter == 0)
		{
			...
			SN_Log(LOGLEVEL_DEBUG, "STATE: Allocated Memory Block [0x%x] (%u Bytes) (%u Bytes Remaining)",
					Result, _Size, State->MemSize - State->UsedMem);
			return Result;
		}
	}

	SN_Log(LOGLEVEL_FATAL, "STATE: Allocator Out of Memory!");
	return 0;
}
...
void* State_ReAlloc(void* _Ptr, u64 _NewSize, u64 _Align)
{
	...
	if (!_Ptr)
	{
		return State_Alloc(_NewSize, _Align);
	}
	...
	u64 OldSize;
	{
		...
		{
			SN_Log(LOGLEVEL_FATAL, "STATE: Failed to realloc mem block! (0x%x Out of Bounds!)", _Ptr);
			return 0;
		}
	...
	if (!Result)
	{
		SN_Log(LOGLEVEL_FATAL, "STATE: Failed to realloc mem block! (%u Bytes From 0x%x To %u Bytes)",
	...
	SN_Log(LOGLEVEL_DEBUG, "STATE: Reallocated Memory Block [0x%x] (%u Bytes) to [0x%x] (%u Bytes)",
...
void State_Free(void* _Ptr)
{
	...
	SN_Log(LOGLEVEL_DEBUG, "STATE: Freed Memory Block [0x%x] (%u Bytes) (%u Bytes Remaining)",
