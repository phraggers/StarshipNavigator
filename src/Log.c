...
static bool Log_ToFile(int _Index)
{
	...
	if (LogFile)
	{
		SN_AllocArray(char, DEF_LOG_BUFFER_SIZE, LogFileBuffer);
...
bool SN_Log(int _Level, const char* _Fmt, ...)
{
	if (!State || !State->Log.Init) return 0;
	...
	if (!State->Log.LogRunning)
	{
		...
		{
			SDL_Thread* Thread = SDL_CreateThread(Log_ThreadFn, "SN_Thread_Log", (void*)&State->Log);
...
bool Log_Init(void)
{
	...
	State->Log.Init = 1;
	SN_Log(LOGLEVEL_INFO, "LOG: Initialized");
