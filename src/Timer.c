...
{
    SN_Log(LOGLEVEL_DEBUG, "TIMER: Initializing, Target %d FPS", _TargetFPS);
...
int Timer_Add(void)
{
    ...
    for (int i = 1; i < DEF_TIMER_SLOTS; i++)
    {
        ...
        {
            SN_Log(LOGLEVEL_DEBUG, "TIMER: Timer[%d] Started", i);
            State->Timer.Timers[i] = SDL_GetPerformanceCounter();
            return i;
        }
    }

    SN_Log(LOGLEVEL_ERROR, "TIMER: Out of Timer slots!");
    return 0;
}
...
r64 Timer_Delta(int _ID)
{
    ...
    if (State->Timer.Timers[_ID] > 0)
    {
        ...
        SN_Log(LOGLEVEL_DEBUG, "TIMER: Timer[%d] Delta[%.02f]", Result);
    }

    else
    {
        SN_Log(LOGLEVEL_ERROR, "TIMER: TimerDelta invalid Timer Slot [%d]", _ID);
    }
...
r64 Timer_End(int _ID)
{
    ...
    if (State->Timer.Timers[_ID] > 0)
    {
        ...
        SN_Log(LOGLEVEL_DEBUG, "TIMER: Timer[%d] Ended[%.02f]", _ID, Result);
        State->Timer.Timers[_ID] = 0;
        return Result;
    }

    SN_Log(LOGLEVEL_ERROR, "TIMER: TimerEnd invalid Timer Slot [%d]", _ID);
