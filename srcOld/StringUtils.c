...
bool Util_RemoveFile(const char* _FilePath)
{
	if (!remove(_FilePath))
	{
		SN_Log(LOGLEVEL_INFO, "UTILS: RemoveFile deleted [%s]", _FilePath);
		return 1;
	}

	else
	{
		SN_Log(LOGLEVEL_ERROR, "UTILS: RemoveFile unable to delete [%s]", _FilePath);
		return 0;
	}
}
...
bool Util_MkDir(const char* _Path)
{
	...
	if (Result)
	{
		SN_Log(LOGLEVEL_ERROR, "UTILS: MkDir Unable to make directory [%s]", _Path);
		return 0;
	}

	else
	{
		SN_Log(LOGLEVEL_INFO, "UTILS: MkDir created [%s]", _Path);
		return 1;
	}
}
...
bool Util_CopyFile(const char* _SrcFile, const char* _DstFile)
{
	...
	if (!SrcFile)
	{
		SN_Log(LOGLEVEL_ERROR, "UTILS: CopyFile failed to open SrcFile [%s]", _SrcFile);
		return 0;
	}
	...
	if (!DstFile)
	{
		SN_Log(LOGLEVEL_ERROR, "UTILS: CopyFile failed to open DstFile [%s]", _DstFile);
		return 0;
	}
	...
	for (i64 i = 0; i < Size; i++)
	{
		...
		if (!SDL_RWread(SrcFile, &c, 1, 1))
		{
			if (i < (Size - 1))
			{
				SN_Log(LOGLEVEL_ERROR, "UTILS: CopyFile failed to read SrcFile [%s]", _SrcFile);
				SDL_RWclose(SrcFile);
				SDL_RWclose(DstFile);
				return 0;
			}
		...
		if (!SDL_RWwrite(DstFile, &c, 1, 1))
		{
			if (i < (Size - 1))
			{
				SN_Log(LOGLEVEL_ERROR, "UTILS: CopyFile failed to write DstFile [%s]", _DstFile);
				SDL_RWclose(SrcFile);
				SDL_RWclose(DstFile);
				return 0;
			}
	...
	SN_Log(LOGLEVEL_INFO, "UTILS: CopyFile copied [%s] to [%s]", _SrcFile, _DstFile);
	return 1;
}
...
bool Util_DirExists(const char* _Path)
{
	...
	{
		SN_Log(LOGLEVEL_DEBUG, "UTILS: DirExists: Yes [%s]", _Path);
		return 1;
	}
	else
	{
		SN_Log(LOGLEVEL_DEBUG, "UTILS: DirExists: No [%s]", _Path);
		return 0;
	}
}